...
class Wallet {
   ...
   calculateWalletBalance(blockchain) {
      ...
      const walletInputTs = transactions.filter(transaction => transaction.inputs.address === this.publicKey);
      let startTime = 0;
      if (walletInputTs.length > 0) {
         const recentInputT = walletInputTs.reduce((prev, cur) => prev.inputs.timestamp ? prev : cur);
